...
class Default_contest extends MY_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Default_contest_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'contest_default/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'contest_default/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'contest_default/index.html';
            $config['first_url'] = base_url() . 'contest_default/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Default_contest_model->total_rows($q);
        $contest = $this->Default_contest_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $this->load->view('contest_default/default_contest_list', $data);
			$this->load->view('../modules/loggedin_template/footer');
    }

    public function read($id) 
    {
        $row = $this->Default_contest_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		//'type' => $row->type,
			...
			$this->load->view('../modules/loggedin_template/header',$data);
            $this->load->view('contest_default/default_contest_read', $data);
				$this->load->view('../modules/loggedin_template/footer',$data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('default_contest'));
        }
    }
    ...
    {       
        $data = array(
            'button' => 'Create',
            'action' => site_url('default_contest/create_action'),
	   ...
	   // 'type' => set_value('type'),
        ...
        $data['matchs'] = $this->Default_contest_model->match_list();
        
		$this->load->view('../modules/loggedin_template/header',$data);
        $this->load->view('contest_default/default_contest_form', $data);
		$this->load->view('../modules/loggedin_template/footer',$data);
    }
    ...
    public function create_action() 
    {
        ...
        //echo "<pre>"; print_r($_POST);die;
            $this->Default_contest_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('default_contest/update_action'),
	...
	//	'type' => $this->input->post('type',TRUE),
            ...
            $this->Default_contest_model->update($this->input->post('contest_id', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Default_contest_model->delete($id);
    ...
    public function winning_information()
    {
        ...
        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Default_contest_model->total_rows_contest($q , $id);
        $contest = $this->Default_contest_model->get_limit_data_contest($config['per_page'], $start, $q ,$id);
       ...
       // echo $this->db->last_query();die();
        ...
        $data = array(
            ...
            $this->load->view('../modules/loggedin_template/header',$data);
        $this->load->view('contest_default/default_winning_information_list', $data);
            $this->load->view('../modules/loggedin_template/footer');
    }
    ...
    public function winnig_information_create()
    {
        ...
        $data = array(
            'button' => 'Create',
            'action' => site_url('default_contest/winnig_information_data/'.$id),
       ...
       // $data['contests'] = $this->Default_contest_model->get_all();
        $this->load->view('../modules/loggedin_template/header',$data);
        $this->load->view('contest_default/default_winnig_information_form', $data);
            $this->load->view('../modules/loggedin_template/footer',$data);
    }
    ...
    public function winnig_information_data()
    {
        ...
        'price' => $this->input->post('price',TRUE),
            ...
            $this->Default_contest_model->insert_winnig($data);
            $this->session->set_flashdata('message', 'Create Record Success');
            redirect(site_url('default_contest/winning_information/'.$id));
        }
    }

    public function winnig_information_read($id)
    {        
        $row = $this->Default_contest_model->get_winninginfo_by_id($id);
        if ($row) {
            $data = array(
        'contest_id' => $row->contest_id,
        'rank' => $row->rank,
        'price' => $row->price,
            ...
            $this->load->view('../modules/loggedin_template/header',$data);
            $this->load->view('contest_default/default_winnig_information_read', $data);
                $this->load->view('../modules/loggedin_template/footer',$data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('default_contest'));
    ...
    public function winnig_information_update($id) 
    {
        ...
        if ($row) {
            ...
            $data = array(
                'button' => 'Update',
                'action' => site_url('default_contest/winnig_information_update_data'),
        'contest_id' => set_value('contest_id', $row->contest_id),
        ...
        'winning_info_id'=>set_value('winning_info_id', $row->winning_info_id),
            ...
            redirect(site_url('default_contest/winnig_information_update/'.$id));
        }
    }
    ...
    public function winnig_information_update_data() 
    {
        ...
        'rank'=>$rank,
            ...
            //print_r($data);die();
            $this->Default_contest_model->update_winninginfo($this->input->post('winning_info_id', TRUE), $data);
            $this->session->set_flashdata('message', 'Update Record Success');
            redirect(site_url('default_contest/winning_information/'.$this->input->post('contest_id', TRUE)));
        }
    }
    ...
    public function winnig_information_delete($id) 
    {
        ...
        if ($row) {
            $this->Default_contest_model->winninginfo_delete($id);
            $this->session->set_flashdata('message', 'Delete Record Success');
            redirect(site_url('default_contest/winning_information/'.$contest_id));
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('default_contest/winning_information/'.$contest_id));
        }
