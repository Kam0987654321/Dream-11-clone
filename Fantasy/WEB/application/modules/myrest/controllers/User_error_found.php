...
{
	public $apikey = 'a9KbzWr6YJcD5z3QUFgC63XyO6H3';
	...
	function match_record_post()
  	{
	    ...
	    foreach ($resp as $key) { 
	       ...
	       // 	$api_url  = "https://rest.entitysport.com/v2/matches/".$key->unique_id."/info?token=".$this->apikey."";
                  $api_url = "https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$key->unique_id."";
		        $ch = curl_init();
 ...
 $api_url = "https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$key->unique_id."";
	...
	function mymatch_record_post()
	{
        ...
        $match_result = $this->Webservice_model->match_score($key->match_id);
       	...
       	$api_url ="https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$U_id."";
	        $ch = curl_init();
	        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	        curl_setopt($ch, CURLOPT_URL,$api_url);
	...
	function joined_contest_post()  
	{
		...
		$this->update_rank_user($re['matchid']);
		...
		$score_match = $this->Webservice_model->match_score($request['contest_id']);
...
// 			$api_url  = "https://rest.entitysport.com/v2/matches/".$score_match->unique_id."/info?token=".$this->apikey."";
			$api_url="https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$score_match->unique_id."";
...
$api_url="https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$score_match->unique_id."";
	...
	function update_match_status_get()
	{
		...
		if(count($response) >0)
		{
			foreach ($response as $resp) {
				...
				$api_url="https://cricapi.com/api/matches?apikey=".$this->apikey."&unique_id=".$resp->unique_id."";
	...
	//echo "<pre>";	print_r($matchs);die();
		...
		{	
			foreach ($matchs as $match) {
				...
				$api_url="https://cricapi.com/api/fantasySquad?apikey=a9KbzWr6YJcD5z3QUFgC63XyO6H3&unique_id=".$id->unique_id."";
	...
	function update_score_points_get()
	{
		...
		{	
			foreach ($matchs as $match) {
				...
				$api_url="https://cricapi.com/api/cricketScore?apikey=".$this->apikey."&unique_id=".$id->unique_id."";
	...
	//echo "<pre>";	print_r($resp);die();
		foreach ($resp as $key) 
		{
			...
			$total_points = $batting + $balling + $fielding + $batting_second + $balling_second + $fielding_second;
...
// 		$api_url  = "http://cricapi.com/api/matches?apikey=a9KbzWr6YJcD5z3QUFgC63XyO6H3";
 $api_url  = "https://cricapi.com/api/matches?apikey=a9KbzWr6YJcD5z3QUFgC63XyO6H3";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL,$api_url);
