...
{
	public $apikey = 'ce7daf8267f3b9fe13ff48998f023d97';
...
//	echo $this->db->last_query();die();
	...
	function my_team_list_post()
	{
		...
		$i = 1;
		foreach ($resp as $value){
			$data = array('match_id'=>$request['match_id']);
			$val = $this->Webservice_model->contest_get($data);
			...
			$value->contest_id = $val->contest_id;
			$value->entry_fees = $val->entry;
	...
	function add_amount_post()
	{
		...
		if($resp !="")
		{
			...
			else if($resp->transaction_status =="TXN_FAILURE") 
			{
				$result = array('status'=>'error','message' =>"TRANSACTION FAILURE",'responsecode'=>'500','data'=>$resp);
			}
		}
...
// 		$api_url  = "http://cricapi.com/api/matches?apikey=a9KbzWr6YJcD5z3QUFgC63XyO6H3";
    ...
    function verifyHash_get($get_input)
    {
      $checked =   $this->check_hash($get_input); 
    ...
    function check_hash($get_input,$salt_key ="e672b597bd4bf61b8b427ec4cd58842809521983")
    { 
      $inp = $get_input;
