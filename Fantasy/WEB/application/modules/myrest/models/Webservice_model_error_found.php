...
class Webservice_model extends CI_Model 
{
     ...
     function login($request, $random)
    {
        if($request['type'] =="Normal")
        {
            $this->db->select('user_id,name,mobile,email,type,verify,referral_code,image');
            $this->db->from('registration');
    ...
    {
        $this->db->select('id,image,teamid,rank,name,user_id,points');  
